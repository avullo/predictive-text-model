...
Here we describe basic data summaries in terms of word and line counts for each of the three files:
...
This exploratory analysis is performed on a random sample which amounts at 5% of the original data. The code used for sampling can be found [here]().
...
# samples <- list(blogs = readLines(as.character(file_paths$sample[1]), skipNul = TRUE),
#                 news = readLines(as.character(file_paths$sample[2]), skipNul = TRUE),
#                 twitter = readLines(as.character(file_paths$sample[3]), skipNul = TRUE))
...
Text is converted to lowercase, and then numbers, punctuaction and high-frequency stop words are removed, including  URLs and bad words:

```{r filtering}
data(stop_words)

# TODO: remove non words
...
# TODO remove URLs, <3 letter words?
...
These type of long-tailed distributions are expected in any given corpus of natural language, a classic example of the Zip's Law (reference?), with many words that occur rarely and fewer words that occur frequently. This is occurring even after removing the most common (i.e. stop) words used in the language.
...
Interestingly, and in line with the previous findings, we can see how we just need a small fraction of the dictionary in order to cover significant portions of all word occurrences: ~4% (~2000) of unqiue words to cover 50%, and ~37% (~19000) of unique words to cover 90% of all word instances in the corpus.
...
### Efficient Random Sampling from Text Files
